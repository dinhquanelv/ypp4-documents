...
WHERE l.Id = @ListId AND ldc.IsVisible = 1
...
WHERE l.Id = @ListId
ORDER BY lr.DisplayOrder, ldc.DisplayOrder
GO

-- case 4: show list view
DECLARE @ListId INT = 1;
SELECT lv.Id AS ListViewId, lv.ViewName
FROM ListView lv
...
JOIN SystemDataType sdt ON sdt.Id = ldc.SystemDataTypeId
JOIN List l ON l.Id = ldc.ListId
WHERE sdt.DataTypeValue = 'DATE' AND l.Id = @ListId
GO
...
SELECT 
	...
	s.ScopeName, 
	s.ScopeDescription
FROM ShareLinkOption slo
JOIN Scope s ON s.Id = slo.ScopeId
WHERE s.ScopeCode = @ScopeCode
GO
